...
import is.hail.keytable.{KeyTable, asc, desc}
...
class SolrSearchEngine(solr: SolrClient) extends SolrEngine(solr) with SearchEngine {
  ...
  override def buildQuery(req: SeqrRequest): SolrQuery = {
    ...
    println(s"override query: $query")
...
class CassLookupEngine(hc: HailContext, address: String, keyspace: String, table: String,
  jsonColumns: Set[String], port: java.lang.Integer = null) extends LookupEngine {
  private val session = CassandraConnector.getSession(address, port)
    ...
    val rows = hc.sc.parallelize(keys)
      .mapPartitions { it =>
        ...
        it.map { key =>
          ...
          val row = results.one()
          CassandraImpex.importRow(tableMetadata, row).asInstanceOf[Row]
        }
      }

    ktToResponse(req, KeyTable(hc, rows, tableType), jsonColumns)
  }
}
...
class VDSHandler(hc: HailContext,
  ...
  def process(req: SeqrRequest): SeqrResponse = {
    var kt2 = req.filterKT(kt)

    req.sort_by.foreach { sb =>
      val fields = sb.obj.map {
        case (f, JString("asc")) =>
          asc(f)
        case (f, JString("desc")) =>
          desc(f)
      }.toArray
      kt2 = kt2.orderBy(fields)
    }
...
class VDSLookupEngine(
  ...
  def lookup(req: SeqrRequest, keys: Seq[Key]): SeqrResponse = {
    val keyIdx = keys.zipWithIndex.toMap

    val filtered = vds.filterVariantsList(
      keys.map(k => Variant(k.chrom, k.start, k.ref, k.alt)).toSet,
      keep = true)

    val kt = req.filterKT(filtered
      .renameSamples(filtered.sampleIds.map(s => s -> q(s)).toMap)
      .makeKT(vExpr, gExpr, seperator = "__"))

    val rows = kt.collect()

    def queryKey(): (Annotation) => Key = {
      val datasetQ = kt.signature.query("dataset_5fid")
      val chromQ = kt.signature.query("chrom")
      val startQ = kt.signature.query("start")
      val refQ = kt.signature.query("ref")
      val altQ = kt.signature.query("alt")

      (a: Annotation) =>
        Key(
          datasetQ(a).asInstanceOf[String],
          chromQ(a).asInstanceOf[String],
          startQ(a).asInstanceOf[Int],
          refQ(a).asInstanceOf[String],
          altQ(a).asInstanceOf[String])
    }

    val sampleFilters = req.sampleFilters()
    val keyQ = queryKey()

    val variants = rows.sortBy(r => keyIdx(keyQ(r)))
      .map(r =>
        rowToVariant(sampleFilters, r, kt.signature))
      .toArray
