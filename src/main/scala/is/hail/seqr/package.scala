...
import is.hail.expr.{JSONAnnotationImpex, TStruct}
...
package object seqr {
  ...
  def rowToVariant(sampleFilters: Option[Set[String]], r: Any, t: TStruct): JObject = {
    ...
    (r.asInstanceOf[Row].toSeq, t.fields).zipped
      .map { case (a, f) =>
        unescapeColumnName(f.name) match {
          case Left(vc) =>
            if (a != null)
              variants += vc -> JSONAnnotationImpex.exportAnnotation(a, f.typ)
          case Right((s, gc)) =>
            // FIXME filter sample
  ...
  def ktToResponse(req: SeqrRequest, kt: KeyTable): SeqrResponse = {
    val variants = req.filterKT(kt)
      .rdd
      .map(r => rowToVariant(req.sampleFilters(), r, kt.signature))
      .collect()
