...
class FilterSuite extends SparkSuite {
  @Test def filterTest() {
    ...
    assert(chr1.filterGenotypes("g.GT.isCalled()").genotypeKT().count() == 9 * 11 - 2)

    val hetOrHomVarOnChr1 = chr1.filterGenotypes("g.GT.isHomRef()", keep = false)

    assert(hetOrHomVarOnChr1.filterGenotypes("g.GT.isCalled()").genotypeKT().count() == 9 + 3 + 3) // remove does not retain the 2 missing genotypes
