...
class AggregatorSuite extends SparkSuite {
  ...
  @Test def testRows() {
    val vds = hc.importVCF("src/test/resources/sample2.vcf")
      ...
      .annotateVariantsExpr(
        """va.test.callrate = gs.fraction(g => g.GT.isCalled()),
          |va.test.AC = gs.map(g => g.GT.nNonRefAlleles()).sum(),
          |va.test.AF = gs.map(g => g.GT.nNonRefAlleles()).stats().sum.toFloat64() / gs.filter(g => g.GT.isCalled()).count() / 2.0,
  ...
  @Test def testColumns() {
    val vds = hc.importVCF("src/test/resources/sample2.vcf")
      .sampleQC()
      .annotateSamplesExpr(
        """sa.test.callrate = gs.fraction(g => g.GT.isCalled()),
          |sa.test.nCalled = gs.filter(g => g.GT.isCalled()).count(),
          |sa.test.nNotCalled = gs.filter(g => !g.GT.isCalled()).count(),
  ...
  @Test def testSum() {
    val p = Prop.forAll(VariantSampleMatrix.gen(hc, VSMSubgen.random)) { vds =>
      val vds2 = vds.splitMulti()
        ...
        .annotateVariantsExpr("va.same = (gs.filter(g => g.GT.isCalled()).count() == 0) || " +
...
va.hist.nGreater == va.nGreater
    ...
    val vds = hc.importVCF("src/test/resources/sample2.vcf").cache()
      .annotateVariantsExpr(
        """va.callStats = gs.map(g => g.GT).callStats(g => v),
          ...
          |va.AN = gs.filter(g => g.GT.isCalled()).count() * 2""".stripMargin)
  ...
  @Test def testQueryGenotypes() {
    Prop.forAll(VariantSampleMatrix.gen(hc, VSMSubgen.random)) { vds =>
      ...
      val (queryResult, t) = vds.queryGenotypes("gs.fraction(g => g.GT.isCalled)")
      ...
      val (queryResult2, t2) = vds.queryGenotypes("gs.fraction(g => (v.start % 2 == 1) && g.GT.isCalled)")
