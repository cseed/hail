...
object LowerMatrixIR {
  ...
  def loweredType(
    typ: MatrixType,
    EntriesSym: Sym = EntriesSym,
    ColsSym: Sym = ColsSym
  ): TableType = TableType(
    rowType = typ.rvRowType.rename(Map(EntriesSym -> EntriesSym)),
    key = typ.rowKey,
    globalType = typ.globalType.appendKey(ColsSym, TArray(typ.colType)))
        ...
        .mapRows(RowSym.insertFields(EntriesSym ->
          GlobalSym(newColIdx).map { kv ~>
            makeStruct(child.typ.entryType.fieldNames.map { s =>
              (s, kv(value).map { i ~> RowSym(EntriesSym)(i)(s) }) }: _*)
          }))
        .mapGlobals(GlobalSym
          .insertFields(ColsSym ->
            GlobalSym(newColIdx).map { kv ~>
              kv(key).insertFields(
                child.typ.colValueStruct.fieldNames.map { s =>
                  (s, kv(value).map(i ~> GlobalSym(ColsSym)(i)(s)))}: _*)
            })
          .dropFields(newColIdx)
        )
  }
  ...
  private[this] def tableRules: PartialFunction[TableIR, TableIR] = {
    ...
    case MatrixColsTable(child) =>
      ...
      let(GlobalAndColsSym ~> lower(child).getGlobals) {
        ...
        makeStruct(RowSym -> sortedCols, GlobalSym -> GlobalAndColsSym.dropFields(ColsSym))
